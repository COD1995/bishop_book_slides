...
**figure 1.2**: Training data (blue circles) and underlying function (green curve).
</div>
</div>
...
**figure 1.3**: Geometric interpretation of the error function.
</div>
</div>
...
<div class="cute-image-container">
  ...
  **figure 1.4**: Effects of varying polynomial orders $m$.
 ...
 **figure 1.5**: RMS error for training and test data across $M$.
...
**figure 1.6**: Overfitting reduces with larger data sets.
</div>
</div>
...
## Regularization in Polynomial Fitting
...
- **Goal**:
  - Control overfitting by adding a penalty term to the error function.

- **Regularized Error Function**:
  $$
  \tilde{E}(\mathbf{w}) = \frac{1}{2} \sum_{n=1}^N \big(y(x_n, \mathbf{w}) - t_n\big)^2 + \frac{\lambda}{2} \|\mathbf{w}\|^2
  $$
  - $\lambda$: Regularization parameter controlling penalty strength.
  - $\|\mathbf{w}\|^2$: Sum of squared coefficients.
...
<div class="cute-image-container">
  <div class="push-pin"></div>
  <img src="imgs/8.png" alt="Regularization Effects" width="600" />
  <div class="small-caption">

**figure 1.7**: Regularized fits for $\ln \lambda = -18$ and $\ln \lambda = 0$.
  </div>
</div>

- **Key Insight**:
  - Larger $\lambda$ values reduce coefficient magnitudes and suppress overfitting.
...
## Coefficient Values with Regularization
...
- **Observation**:
  - As $\lambda$ increases, coefficients $\mathbf{w}^*$ decrease in magnitude.
  - Regularization limits large oscillations in fitted functions.

- **Table 1.2**:
  - Coefficient values for different $\ln \lambda$:
    - $\ln \lambda = -\infty$: No regularization.
    - $\ln \lambda = 0$: Strong regularization.
...
<div class="cute-image-container">
  <div class="push-pin"></div>
  <img src="imgs/9.png" alt="Regularization Effects" width="600" />
  <div class="small-caption">

Refer to **Table 1.2** for detailed coefficient values.
  </div>
</div>
...
## Effect of Regularization on RMS Error
...
- **RMS Error Trends**:
  - Training error increases with larger $\lambda$ due to stronger regularization.
  - Test error decreases initially but increases for excessively large $\lambda$.

- **Figure 1.8**:
  - Plots $\ln \lambda$ vs. $E_{RMS}$ for training and test data.
...
<div class="cute-image-container">
  <div class="push-pin"></div>
  <img src="imgs/10.png" alt="RMS Error vs Regularization" width="600" />
  <div class="small-caption">

**figure 1.8**: RMS error as a function of $\ln \lambda$.
  </div>
</div>
